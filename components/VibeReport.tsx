...
function Stat({
  ...
  return (
    <div className={`relative rounded-xl border p-5 overflow-hidden ${accent}`}>
      ...
      {trend && (
        <span className="absolute top-4 right-4 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
          {trend}
        </span>
      )}
    </div>
      ...
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        <div>
          <h3 className="text-[13px] font-bold text-slate-800">
            ...
            <span className="text-slate-400 font-normal ml-1.5">// Vibe Report</span>
          </h3>
          <p className="text-[11px] text-slate-500 mt-0.5">{creatorHandle} · {totalGraded} comments graded</p>
        </div>
        <div className="text-[11px] text-violet-700 bg-violet-50 border border-violet-200 px-3 py-1 rounded-full font-medium">
          HumaneBench v3.0
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Stat
            ...
            sub={avgScoreLabel}
            accent="border-slate-100 bg-slate-50"
          />
          <Stat
        ...
        {/* Grade distribution */}
        <div>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Grade Distribution</p>
            ...
            {GRADE_ORDER.map((grade) => {
              ...
              return (
                <span key={grade} className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  ...
                  {grade} <span className="text-slate-400">({count}, {pct}%)</span>
                </span>
              )
            })}
          </div>
        ...
        <div className="rounded-xl border border-violet-200 bg-violet-50 px-5 py-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[11px] font-bold text-violet-700 uppercase tracking-widest">Recommended Action</p>
          </div>
